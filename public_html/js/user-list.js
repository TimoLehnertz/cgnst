...
function getStruct(){
    ...
    $(".group-list__group").each((i, e)=>{
        let group = {users:[]};
        $(e).find(".user-list__user").each((i, userElement)=>{
            let user = {
                username: $(userElement).find(".user-list__user__name").text(),
...
function addUsersToList(users){
    for (const user of users) {
        $(".user-list__list").append(getUserElement(user));
    }
}

function getUserElement(user){
    const element = $(`
    <div class="user-list__user" draggable="true">
        <span class="user-list__user__name">${user.username}</span>
    </div>`);
...
function groupElementHasUser(groupElement, username){
    success = false;
    groupElement.find(".user-list__user__name").each((i,e) => {
        if($(e).text() == username){
            success = true;
        }
    });
...
function getDragElementName(){
    if(dragElement != undefined){
        return dragElement.find(".user-list__user__name").text();
    }
}